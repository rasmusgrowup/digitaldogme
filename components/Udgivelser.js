...
export default function Publikationer({arr, types}) {
    ...
    //const [ kategorier, setKategorier ] = useState(types.enumValues); // Enable to not include cases
    const [kategorier, setKategorier] = useState([...types.enumValues, {name: "Case"}].sort((a, b) => a.name > b.name ? 1 : -1)); // Enable to include cases
                ...
                <div className={styles.inner}>
                    {kategorier &&
                        <div className={styles.filter}>
                            ...
                            {kategorier.map((kategori, i) => (
                                <button key={i}
                                        className={`${styles.kategoriBtn} ${kategori.name === filter && `${styles.selected}`}`}
                        ...
                        {grid.map((publikation, i) => {
                            return (
                                <div className={styles.publikation} key={i}>
                                    <Link
                                        href={publikation.__typename === "Case" ? `${`/cases/${publikation.slug}`}` : `${`/viden/${publikation.slug}`}`}
                                        key={i} passHref>
                                        <a>
                    ...
                    {itemsLoaded < filteredArr.length &&
                        <div className={styles.loaderBtn}>
                            <button onClick={() => setItemsLoaded(itemsLoaded + increaseBy)}>Vis
                                flere <sup>({itemsLoaded}/{filteredArr.length})</sup></button>
                        </div>
                    }
                </div>
            </section>
